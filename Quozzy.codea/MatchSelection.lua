    ...
    GKTurnBasedMatch:loadMatchesWithCompletionHandler_(function(o__matches, o__err)
      objc.async(function()
        local okInner, errInner = pcall(function()
          ...
          devLog("vs: matches list refreshed", "count=", #list, "actionable=", vsHasActionable, "reason=", reason or "?")
        end)
        if not okInner then
          vsListLoading = false
...
function _drawVsFriendsList(g)
  ...
  local disclaimer = "Because Vivaldiku enables sending text to other players you may only play against existing Game Center friends."
